...
- 此功能目前正在开发中，请稍后再查看对齐跟踪功能。`,
    model: getMastraModel(false), // No tools currently, Mastra handles fallback automatically
  });
...
export async function alignmentAgent(
  ...
  console.log(
    `[Alignment] Memory context: memoryThread=${memoryThread}, memoryResource=${memoryResource}`
  );
 ...
 */
export function getAlignmentAgentMastra(): Agent {
  initializeAgent();
