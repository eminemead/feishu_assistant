...
export function extractFeishuUserId(message: any, data: any): string | null {
  ...
  // open_id is opaque like "ou_xxx" which doesn't map to GitLab usernames
  
  // Check message.sender first (most common location)
  if (message?.sender) {
    // Prefer user_id (maps to GitLab username) over open_id
    if (message.sender.sender_id?.user_id && typeof message.sender.sender_id.user_id === 'string') {
    ...
    // Fall back to open_id (for RLS/memory purposes when user_id unavailable)
  ...
  if (data?.sender) {
    // Same priority: user_id > open_id
    if (data.sender.sender_id?.user_id && typeof data.sender.sender_id.user_id === 'string') {
      return data.sender.sender_id.user_id;
    }
