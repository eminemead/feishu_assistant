# Documentation Index

This directory contains all project documentation organized by category.

## 📚 Documentation Structure

### Setup & Configuration
- [DSPyground Setup](./setup/dspyground-setup.md) - Guide for setting up and using DSPyground for prompt optimization
- [AI SDK Devtools Setup](./setup/devtools-setup.md) - Real-time monitoring and debugging for agent system
- [Devtools Walkthrough](./setup/devtools-walkthrough.md) - Step-by-step guide on using devtools
- [Devtools Observability](./setup/devtools-observability.md) - Using devtools as production observability tool
- [Observability Evaluation](./setup/observability-evaluation.md) - Comparison of observability tools (DSPyground vs Evalite)

### Architecture & Design
- [Agent Architecture](./architecture/agent-architecture.md) - Multi-agent system design and routing logic
- [Routing Logic](./architecture/routing-logic.md) - How queries are routed to specialist agents
- [Handoff System](./architecture/handoff-system.md) - Agent handoff mechanism and verification

### Implementation Guides
- [OKR Tool Implementation](./implementation/okr-tool.md) - OKR metrics analysis tool implementation
- [Visualization Implementation](./implementation/visualization.md) - OKR visualization features
- [Chinese Language Support](./implementation/chinese-support.md) - Chinese language handling
- [Artifacts Integration Evaluation](./implementation/artifacts-evaluation.md) - Evaluation of leveraging @ai-sdk-tools/artifacts
- [Cache Integration](./implementation/cache-integration.md) - Using @ai-sdk-tools/cache for performance optimization
- [Build Optimization](./implementation/build-optimization.md) - TypeScript build time optimizations

### Testing
- [Testing Guide](../TESTING.md) - Main testing documentation (kept at root for visibility)
- [Test Quick Start](./testing/test-quick-start.md) - Quick start guide for tests

### Verification & Verification
- [Feishu Image API Verification](./verification/feishu-image-api.md) - Image API setup verification
- [Handoff Verification](./verification/handoff-verification.md) - Agent handoff verification

## 📖 Quick Links

- [Main README](../README.md) - Project overview and getting started
- [Agent README](../lib/agents/README.md) - Agent-specific documentation
- [Visualization README](../lib/visualization/README.md) - Visualization features

